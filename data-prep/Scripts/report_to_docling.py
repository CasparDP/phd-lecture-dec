...
def main():
    ...
    try:
        ...
        # Save to paper_summaries directory as .qmd file
        repo_root = Path(__file__).parent.parent.parent
        paper_summaries_dir = repo_root / "paper_summaries"

        pdf_path = Path(args.file)
        pdf_stem = pdf_path.stem
        output_dir = paper_summaries_dir / pdf_stem
        output_dir.mkdir(parents=True, exist_ok=True)
...
:::{{.callout-note}}
# AI Generated Summary
Generated by Paper Summarizer using Ollama (Model: {args.model})
:::
"""

        qmd_file = output_dir / f"{pdf_stem}_summary.qmd"
        with open(qmd_file, "w", encoding='utf-8') as f:
            f.write(qmd_content)

        logger.info(f"✓ Summary saved to: {qmd_file}")

        # Export if requested (additional export option)
        if args.export:
            if args.export == 'auto':
