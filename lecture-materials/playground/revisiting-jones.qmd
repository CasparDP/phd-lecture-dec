...
- $Total accruals_{it}$ = total accruals for firm i at time t
...
Discretionary accruals: $DA_{it}$ = residual from firm i's time-series regression
:::
...
$DA_{it}$ = residual from industry-year cross-sectional regression
...
$$\Delta DA_{it} = \beta_1Treated_i + \alpha_{fyear}+ \gamma_{cohort}+ \epsilon_{it}$$

Where:

-  $\Delta(DA_{it}) = DA_{it} - DA_{i,t-1}$ is the change in discretionary accruals for firm i at time t
